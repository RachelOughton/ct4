...
This page contains the notes for Clinical Trials IV 2025-26. You can also download the PDF version (see the icon at the top left). If you notice any typos, mistakes or places that are unclear, please do let me know!
...
Our lectures are 3 pm on Mondays and 10 am on Wednesdays, both in W414 (this is the Appleby Building, between Physics and the main library).

\subsection*{Computer classes}\label{computer-classes}
\addcontentsline{toc}{subsection}{Computer classes}

We have four computer practicals for this module. They are 12-1 pm on the Fridays of weeks 13, 15, 17 and 19. These classes are in MCS 3097.

\subsection*{Office Hour}\label{office-hour}
\addcontentsline{toc}{subsection}{Office Hour}

The office hour will be every Monday, 10-11am, in my office (MCS 3016).

\subsection*{Assessment}\label{assessment}
\addcontentsline{toc}{subsection}{Assessment}

This module is assessed through two equally weighted pieces of coursework. The first will be assigned on Wednesday 11th February (due 2nd March), the second on Wednesday 18th March (due 4th May). I have given you more time than you need, so that you can plan ahead. If you will need an extension please contact the maths teaching and learning team (\href{mailto:maths.teaching@durham.ac.uk}{\nolinkurl{maths.teaching@durham.ac.uk}}) as soon as possible. \href{https://durhamuniversity.sharepoint.com/teams/extensionsselfcertification/extensions}{This page} explains the university policy on extensions for coursework.
...
I will share links along the way for relevant resources like articles and podcast episodes, which I hope will help you to get a feel for the topic more generally.
...
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/unnamed-chunk-2-1.pdf}}
\end{example}
...
\section{The structure of a clinical trial}\label{primout}
...
Knowing exactly what type of patients were recruited into the trial is also key when generalising the results to the population. If the trial recruited males aged 55-70, we cannot confidently conclude that the results will apply to a female aged 26.
...
\subsection*{The primary outcome}\label{the-primary-outcome}
\addcontentsline{toc}{subsection}{The primary outcome}
...
Now that the trial has been run, we have two sets of measurements (in particular, measurements of the primary outcome): one for the treatment group and one for the control group. But guess what?! Comparing these and coming to a conclusion about the effect of the treatment is a statistical question.

\subsection*{Trial timeline}\label{trial-timeline}
\addcontentsline{toc}{subsection}{Trial timeline}

Now that we've thought (very briefly) about the main stages in a clinical trial, we can look at how they fit together.

\includegraphics[width=5.93in]{images/timeline}

As we see above, the recruitment, allocation and trial phases overlap. Once a participant is recruited into the trial, they are (usually) allocated to the control or treatment group very soon after, and then their course of treatment (whichever it is) begins. The recruitment phase will continue until enough participants have been recruited (or sometimes until some deadline). Depending on how long this takes, and how long the actual trial phase lasts, some participants may have finished their course of treatment before others have even been allocated. This has implications for the allocation in particular, since we can't just allocate all the participants at once.

Results are collected during the trial, and once everyone has finished their course of treatment the analysis phase can begin.

\subsection*{Trial protocol and analysis plan}\label{trial-protocol-and-analysis-plan}
\addcontentsline{toc}{subsection}{Trial protocol and analysis plan}

Before doing any of the above stages, it is important to write a protocol and analysis plan. This is usually a requirement of any clinical body (and indeed some other bodies who run trials, like the Educational Endowment Foundation).

The protocol and analysis plan will include the rationale for the trial itself, and for the choice of primary outcome variable. It will also include practical details such as eligibility criteria, recruitment plans and methods for keeping participants (and probably clinicians) blind to who is in which group.

There will also be lots of detail about the statistical plan, for example the sample size will be determined using existing knowledge about the primary outcome and details about the planned analysis. The planned approach to allocation will be detailed, and finally the statistical analysis plan for the primary outcome and any subgroup or secondary analyses. There should be contingencies for possible issues, for example ``If 20\% or more of the primary outcome values are missing, we will use X missing data approach''.

\begin{example}
\emph{Accelerated Reader (EEF, 2018)}
Accelerated Reader is a web-based programme that encourages children to read for pleasure. You can find the protocol at \url{https://educationendowmentfoundation.org.uk/projects-and-evaluation/projects/accelerated-reader-effectiveness-trial}.

The protocol for this trial lays out (among other things)

\begin{itemize}
\tightlist
\item
  the design of the trial: cluster-randomised controlled trial
\item
  School eligibility: approximately half to be located in the North East, not used AR in last two years, containing years 3 to 6, no special or independent schools
\item
  Pupil eligibility within schools: depends on ability to take a STAR reading test, parental consent
\item
  Strategy for recruitment
\item
  the mode of random allocation, and which variables will be taken into account and how
\item
  Primary outcome measure (different for different year groups)
\item
  Sample size calculations (informed by lots of the other points)
\item
  Statistical modelling plan: use both multilevel and school-dummy fixed effect models (don't worry if you don't know these!)
\item
  How significance of results will be determined based on the model fit.
\end{itemize}

The evaluation report then follows this plan exactly, unless there is very good reason not to.
\end{example}
...
\item
  If we recruit too many patients (ie. we would be sufficiently likely to reach a conclusion with fewer) then we have subjected more patients than necessary to an inferior treatment (assuming one treatment was better), and have probably taken up more time and resources than was necessary.
\end{enumerate}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/t31-onesided-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/t31-twosided-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/unnamed-chunk-4-1.pdf}}

In R, we have \(\Phi\left(z_{\alpha/2}\right) = \texttt{pnorm}\left(z_{\alpha/2}\right)\) and \(z_{\alpha/2} = \texttt{qnorm}\left(\Phi\left(z_{\alpha/2}\right)\right)\). \texttt{qnorm} is the quantile and \texttt{pnorm} is the cumulative distribution function. So,
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/accepth0-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/accepth0-1-1.pdf}}
...
For given values of \(\alpha\), \(\sigma\) and \(\lambda\left(n,m\right)\), we can calculate the power as a function of \(\tau\) by finding the area of the distribution of \(D\) under \(H_1\) for which we accept \(H_1\).
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/powercurve-1.pdf}}
...
Suppose a trial investigates a drug that turns out to have a much stronger effect on male patients than on female patients. The cohort of recruited participants are randomised into treatment and control groups, and it happens that there is a much smaller proportion of female patients in one group than the other. This will distort the estimated treatment effect.
...
From here, we can calculate the expected value of the estimated treatment effect \(\hat\tau\) as (substituting our equation for \(b\) and rearranging):
...
\[E\left(\hat\tau\right) = E\left(X\mid{A,N}\right)- E\left(X\mid{A,S}\right) < \tau_N - \tau_S.\]
...
In this chapter, and in the computer practical on allocation, we will study the behaviour of various allocation methods by implementing them many times. This gives us an idea of how variable the results are, especially in terms of being likely to introduce bias or reduce power. In almost all real trials, patients are allocated and begin treatment as they arrive, and this may happen over the course of weeks or months. The allocation that is generated is the final allocation. It is therefore important to minimise the probability of it being a poor one!
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/srs-alloc-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/srsalloc200-1.pdf}}
...
Biased coin designs and urn schemes both work by adjusting the probabilities of allocation according to the balance of the design so far, such that a participant is less likely to be assigned to an over-represented group.

\subsubsection{Biased coin designs}\label{biased-coin-designs}

The biased coin design was introduced by \citet{efron1971forcing}, with the aim of ensuring balance whilst not becoming vulnerable to various forms of experimental bias. \citet{efron1971forcing} suggested the biased coin design be used to achieve balance in terms of patient characteristics (eg. age group, sex, disease state etc.), but in this section we will think about the whole cohort (the maths for a subgroup would be the same).
...
\[ D\left(n\right) = N_T\left(n\right) - N_C\left(n\right).\]
...
If \(\lvert D\left(n\right) \rvert = 0\), ie. the scheme is in exact balance after \(n\) allocations, then we must have \(\lvert D\left(n+1\right)\rvert = 1\).
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/biasedcoin-p2thirds-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/p-nearlyhalf-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/p-nearlyone-1.pdf}}
...
\[D\left(n\right) = N_T\left(n\right) - N_C\left(n\right) = 2N_T\left(n\right) - n = n - 2N_C\left(n\right).\]
...
\[
  \begin{aligned}
p\left(\lvert D\left(n+1\right)\rvert = j-1 \mid \underbrace{D\left(n\right)=j>0}_{\text{More in T}}\right) & = \frac{r + N_C\left(n\right)s}{2r + ns} \\
...
\[
  \begin{aligned}
p\left(\lvert D\left(n+1\right)\rvert = j-1 \mid \underbrace{D\left(n\right)=j<0}_{\text{More in C}}\right) & = \frac{r + N_T\left(n\right)s}{2r + ns} \\
& = \frac{r + \frac{1}{2}\left(n - D\left(n\right)\right)s}{2r + ns} \\
& = \frac{1}{2} - \frac{D\left(n\right)s}{2\left(2r + ns\right)}\\
& = \frac{1}{2} + \frac{\lvert D\left(n\right)\rvert s}{2\left(2r + ns\right)}.
\end{aligned}
...
\[
  \begin{aligned}
P\big(\lvert D\left(n+1\right) \rvert = 1 \mid \lvert D\left(n\right)\rvert=0\big)& = 1\\
P\big(\lvert D\left(n+1\right) \rvert = j+ 1 \mid \lvert D\left(n\right)\rvert=j>0\big)& = \frac{1}{2}  - \frac{\lvert D\left(n\right)\rvert s}{2\left(2r + ns\right)}\\
P\big(\lvert D\left(n+1\right) \rvert = j-1 \mid \lvert D\left(n\right)\rvert=j>0\big)& = \frac{1}{2}  + \frac{\lvert D\left(n\right)\rvert s}{2\left(2r + ns\right)}
\end{aligned}
\]
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/urn11-1.pdf}}
\caption{\label{fig:urn11}Four realisations of absolute imbalance for r=1, s=1, N=50.}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/urn18-1.pdf}}
\caption{\label{fig:urn18}Four realisations of absolute imbalance for r=1, s=8, N=50.}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/urn81-1.pdf}}
...
Minimization was first proposed by \citet{taves1974minimization}, then shortly after by \citet{pocock1975sequential} and \citet{freedman1976use}. The aim of minimization is to minimize the difference between the two groups. Although the method was developed in the seventies, it has only gained popularity relatively recently, mainly as computers have become widely available.

The people running the trial must first specify all of the factors they would like to be balanced between the two groups. These should be any variables that are thought to possibly affect the outcome.
...
Suppose there are several prognostic factors over which we require balance, and that these factors have \(I, J, K, ...\) levels. In the example above, there would be \(I=2,\; J=2,\; K=3,\; L=2\). Note that this equates to 24 strata.
...
An issue here is that only the marginal distributions are compared for similarity. Consider the following (somewhat extreme and minimalistic) scenario. A study aims to investigate the effect of some treatment, and to balance for gender and age in their allocation, resulting in the following summary table.
...
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/unnamed-chunk-10-1.pdf}}
...
\end{aligned}
\]
This enables us to do an independent two-sample \(t\)-test, by computing the standardized measure of effect size we found in Section \ref{sec-measDcont}. We find the test statistic

\[
  \begin{aligned}
D & = \frac{\bar{X_C} - \bar{X_T}}{s_p\sqrt{\frac{1}{n_C} + \frac{1}{n_T}}}\\
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/ttestcapt-1.pdf}}
\caption{\label{fig:ttestcapt}The distribution \(t_{14}\), with \(D=1.65\) shown by the dashed line and the `more extreme' areas shaded.}
\end{figure}

The dashed line in Figure \ref{fig:ttestcapt} is at \(D=1.65\), and the red shaded areas show anywhere `at least as extreme'. We can find the area (ie. the probability of anything at least as extreme as our found value) in R by
...
Using this to work out the variance of the estimators we find that for comparing means, assuming two equally sized groups of size \(N\), we have

\[\operatorname{var}\left(\hat{\tau}_{\text{out}}\right) = \operatorname{var}\left(\bar{x}_T - \bar{x}_C\right) = \frac{2\sigma^2}{N}.\]

whereas for comparing differences from baseline

\[\operatorname{var}\left(\hat{\tau}_{\text{diff}}\right) = \operatorname{var}\left[\left(\overline{X_T-B_T}\right) - \left(\overline{X_C - B_C}\right)\right] = 2\left(1-\rho\right)\left(\frac{2\sigma^2}{N}\right).\]
...
Intuitively, this seems reasonable: if the correlation between baseline and outcome measurements is very strong, then we can remove some of the variability between participants by taking into account their baseline measurement. However, if the correlation is weak, then by including the baseline in the analysis we are essentially just introducing noise. If the correlation is negative (very unusual in practice) then \(\operatorname{var}\left(\hat{\tau}_{\text{diff}}\right)\) is much larger than \(\operatorname{var}\left(\hat{\tau}_{\text{out}}\right)\) - this also makes sense intuitively because we aren't allowing the coefficient of the baseline measurement to reflect the relationship between baseline and outcome.
...
Sometimes the analysis performed on a dataset is rather spurious, but it isn't always immediately obvious why. We'll look at one example now, because it is sometimes done.
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/hommel-1.pdf}}
...
Suppose the outcome for a clinical trial is \(X\), which has mean \(\mu\) in the control group (C) and mean \(\mu+\tau\) in the treatment group (T), and as usual our aim is to determine the extent of \(\tau\), the treatment effect. We suppose also that \(X\) has variance \(\sigma^2\) in both groups.

The same quantity is measured at the start of the trial, and this is the baseline \(B\), which we can assume to have true mean \(\mu_B\) in both groups (because of randomisation) and variance \(\sigma^2\). We also assume that the true correlation between \(B\) and \(X\) is \(\rho\) in each group. Finally, we assume that both trial arms are of size \(N\). We therefore have \(2N\) patients, and so we observe baseline measurements \(b_1,\,b_2,\ldots,b_{2N}\).
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/bvnplot-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/unnamed-chunk-15-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/unnamed-chunk-16-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/lmstressresid1-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/lmstressresid2-1.pdf}}
...
As you'll have seen if you read \citet{kendall2003designing} (for formative assignment 1), we should have everything in place, including a statistical analysis plan, \textbf{before} the trial. We should already know which covariates we plan to include in our model, and how we are going to assess the fit. `Trawling' for the best possible model by trying lots of different things (and inevitably settling on the one that leads to the most significant conclusion) is poor practice, and can increase the type I error rate (\(\alpha\)).

That said, it is crucial to check the model diagnostics, residual plots and so on, to make sure that the model is an appropriate one for the data. If there are problems (eg. if it appears that a particular covariate isn't having a linear effect, or if there seems to be an interaction between two covariates) then you need to try different things until you get a good fit. This is different from p-fishing (described above), because you aren't doing it to minimise the p-value. Indeed, the p-value might be higher for the model that best fits the data.

The way we've gone through analysis methods in this Section was to demonstrate and compare the different methods. Proceeding in the way we have, trying lots of different models, when analysing and writing up a trial would be very poor practice!
...
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/unnamed-chunk-26-1.pdf}}
\end{example}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/unnamed-chunk-28-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/newc1-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/unnamed-chunk-32-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/unnamed-chunk-35-1.pdf}}
...
Since neither \texttt{age} nor \texttt{age\^{}2} appear influential, and the AIC and residual deviance values are almost identical, we'll remove it and keep the other covariates.
...
Recall that \(\tau\) is the log of the odd ratio, so we can make this (slightly) more intuitive by finding a confidence interval for the odds ratio itself:

\[\left(\exp(-1.26),\;\exp(-0.240)\right) = \left(0.28,\,0.79\right).\]
...
Being more methodical we can collect these into a table, and use \texttt{predict.glm}. This also has the advantage that we can find the standard error for the log odds ratio for each category. Since the site 3 and 4 coefficents are not significant (mainly due to a lack of data), we will focus only on the site 1 and 2 participants.

\begin{Shaded}
\begin{Highlighting}[]
\DocumentationTok{\#\# Create data frame with every combination of factors (only showing sites 1 and 2)}
\NormalTok{df\_indo }\OtherTok{=} \FunctionTok{data.frame}\NormalTok{(}
  \AttributeTok{site =} \FunctionTok{rep}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\FunctionTok{rep}\NormalTok{(}\StringTok{"2\_IU"}\NormalTok{, }\DecValTok{5}\NormalTok{), }\FunctionTok{rep}\NormalTok{(}\StringTok{"1\_UM"}\NormalTok{, }\DecValTok{5}\NormalTok{)),}\DecValTok{2}\NormalTok{),}
  \AttributeTok{risk =} \FunctionTok{rep}\NormalTok{(}\DecValTok{1}\SpecialCharTok{:}\DecValTok{5}\NormalTok{, }\DecValTok{4}\NormalTok{),}
  \AttributeTok{rx =} \FunctionTok{rep}\NormalTok{(}\FunctionTok{c}\NormalTok{(}\StringTok{"0\_placebo"}\NormalTok{, }\StringTok{"1\_indomethacin"}\NormalTok{), }\FunctionTok{c}\NormalTok{(}\DecValTok{10}\NormalTok{,}\DecValTok{10}\NormalTok{))}
\NormalTok{)}

\DocumentationTok{\#\# Find and transform fitted value}

\NormalTok{df\_indo}\SpecialCharTok{$}\NormalTok{logodds }\OtherTok{=} \FunctionTok{predict}\NormalTok{(glm\_indo, }\AttributeTok{newdata=}\NormalTok{df\_indo, }\AttributeTok{se.fit =}\NormalTok{ T)}\SpecialCharTok{$}\NormalTok{fit}
\NormalTok{df\_indo}\SpecialCharTok{$}\NormalTok{logodds\_se }\OtherTok{=} \FunctionTok{predict}\NormalTok{(glm\_indo, }\AttributeTok{newdata=}\NormalTok{df\_indo, }\AttributeTok{se.fit =}\NormalTok{ T)}\SpecialCharTok{$}\NormalTok{se.fit}
\NormalTok{df\_indo}\SpecialCharTok{$}\NormalTok{logor\_lower }\OtherTok{=}\NormalTok{ df\_indo}\SpecialCharTok{$}\NormalTok{logodds }\SpecialCharTok{{-}} \FunctionTok{qnorm}\NormalTok{(}\FloatTok{0.975}\NormalTok{)}\SpecialCharTok{*}\NormalTok{df\_indo}\SpecialCharTok{$}\NormalTok{logodds\_se}
\NormalTok{df\_indo}\SpecialCharTok{$}\NormalTok{logor\_upper }\OtherTok{=}\NormalTok{ df\_indo}\SpecialCharTok{$}\NormalTok{logodds }\SpecialCharTok{+} \FunctionTok{qnorm}\NormalTok{(}\FloatTok{0.975}\NormalTok{)}\SpecialCharTok{*}\NormalTok{df\_indo}\SpecialCharTok{$}\NormalTok{logodds\_se}

\NormalTok{df\_indo}\SpecialCharTok{$}\NormalTok{odds }\OtherTok{=} \FunctionTok{exp}\NormalTok{(df\_indo}\SpecialCharTok{$}\NormalTok{logodds)}
\NormalTok{df\_indo}\SpecialCharTok{$}\NormalTok{odds\_lower }\OtherTok{=} \FunctionTok{exp}\NormalTok{(df\_indo}\SpecialCharTok{$}\NormalTok{logor\_lower)}
\NormalTok{df\_indo}\SpecialCharTok{$}\NormalTok{odds\_upper }\OtherTok{=} \FunctionTok{exp}\NormalTok{(df\_indo}\SpecialCharTok{$}\NormalTok{logor\_upper)}

\NormalTok{df\_indo}\SpecialCharTok{$}\NormalTok{prob\_lower }\OtherTok{=}\NormalTok{ df\_indo}\SpecialCharTok{$}\NormalTok{odds\_lower}\SpecialCharTok{/}\NormalTok{(}\DecValTok{1}\SpecialCharTok{+}\NormalTok{df\_indo}\SpecialCharTok{$}\NormalTok{odds\_lower)}
\NormalTok{df\_indo}\SpecialCharTok{$}\NormalTok{prob }\OtherTok{=}\NormalTok{ df\_indo}\SpecialCharTok{$}\NormalTok{odds}\SpecialCharTok{/}\NormalTok{(}\DecValTok{1}\SpecialCharTok{+}\NormalTok{df\_indo}\SpecialCharTok{$}\NormalTok{odds)}
\NormalTok{df\_indo}\SpecialCharTok{$}\NormalTok{prob\_upper }\OtherTok{=}\NormalTok{ df\_indo}\SpecialCharTok{$}\NormalTok{odds\_upper}\SpecialCharTok{/}\NormalTok{(}\DecValTok{1}\SpecialCharTok{+}\NormalTok{df\_indo}\SpecialCharTok{$}\NormalTok{odds\_upper)}


\NormalTok{df\_indo\_round2 }\OtherTok{\textless{}{-}}\NormalTok{ df\_indo }\SpecialCharTok{\%\textgreater{}\%}                   \CommentTok{\# Using dplyr functions}
  \FunctionTok{mutate\_if}\NormalTok{(is.numeric,}
\NormalTok{            round,}
            \AttributeTok{digits =} \DecValTok{2}\NormalTok{)}
\NormalTok{df\_indo\_round2 }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{kable}\NormalTok{() }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{kable\_styling}\NormalTok{(}\StringTok{"striped"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\hline
site & risk & rx & logodds & logodds\_se & logor\_lower & logor\_upper & odds & odds\_lower & odds\_upper & prob\_lower & prob & prob\_upper\\
\hline
2\_IU & 1 & 0\_placebo & -2.87 & 0.32 & -3.48 & -2.25 & 0.06 & 0.03 & 0.11 & 0.03 & 0.05 & 0.10\\
\hline
2\_IU & 2 & 0\_placebo & -2.28 & 0.22 & -2.72 & -1.85 & 0.10 & 0.07 & 0.16 & 0.06 & 0.09 & 0.14\\
\hline
2\_IU & 3 & 0\_placebo & -1.70 & 0.19 & -2.08 & -1.32 & 0.18 & 0.13 & 0.27 & 0.11 & 0.15 & 0.21\\
\hline
2\_IU & 4 & 0\_placebo & -1.11 & 0.26 & -1.61 & -0.61 & 0.33 & 0.20 & 0.54 & 0.17 & 0.25 & 0.35\\
\hline
2\_IU & 5 & 0\_placebo & -0.53 & 0.36 & -1.24 & 0.18 & 0.59 & 0.29 & 1.20 & 0.22 & 0.37 & 0.55\\
\hline
1\_UM & 1 & 0\_placebo & -1.65 & 0.28 & -2.19 & -1.10 & 0.19 & 0.11 & 0.33 & 0.10 & 0.16 & 0.25\\
\hline
1\_UM & 2 & 0\_placebo & -1.06 & 0.22 & -1.49 & -0.63 & 0.35 & 0.22 & 0.53 & 0.18 & 0.26 & 0.35\\
\hline
1\_UM & 3 & 0\_placebo & -0.48 & 0.24 & -0.95 & 0.00 & 0.62 & 0.39 & 1.00 & 0.28 & 0.38 & 0.50\\
\hline
1\_UM & 4 & 0\_placebo & 0.11 & 0.33 & -0.54 & 0.75 & 1.11 & 0.59 & 2.12 & 0.37 & 0.53 & 0.68\\
\hline
1\_UM & 5 & 0\_placebo & 0.69 & 0.44 & -0.17 & 1.56 & 2.00 & 0.84 & 4.76 & 0.46 & 0.67 & 0.83\\
\hline
2\_IU & 1 & 1\_indomethacin & -3.62 & 0.36 & -4.32 & -2.92 & 0.03 & 0.01 & 0.05 & 0.01 & 0.03 & 0.05\\
\hline
2\_IU & 2 & 1\_indomethacin & -3.03 & 0.27 & -3.56 & -2.51 & 0.05 & 0.03 & 0.08 & 0.03 & 0.05 & 0.08\\
\hline
2\_IU & 3 & 1\_indomethacin & -2.45 & 0.24 & -2.91 & -1.99 & 0.09 & 0.05 & 0.14 & 0.05 & 0.08 & 0.12\\
\hline
2\_IU & 4 & 1\_indomethacin & -1.86 & 0.28 & -2.41 & -1.32 & 0.15 & 0.09 & 0.27 & 0.08 & 0.13 & 0.21\\
\hline
2\_IU & 5 & 1\_indomethacin & -1.28 & 0.37 & -2.01 & -0.55 & 0.28 & 0.13 & 0.58 & 0.12 & 0.22 & 0.37\\
\hline
1\_UM & 1 & 1\_indomethacin & -2.40 & 0.32 & -3.02 & -1.77 & 0.09 & 0.05 & 0.17 & 0.05 & 0.08 & 0.15\\
\hline
1\_UM & 2 & 1\_indomethacin & -1.81 & 0.26 & -2.32 & -1.30 & 0.16 & 0.10 & 0.27 & 0.09 & 0.14 & 0.21\\
\hline
1\_UM & 3 & 1\_indomethacin & -1.23 & 0.27 & -1.76 & -0.70 & 0.29 & 0.17 & 0.50 & 0.15 & 0.23 & 0.33\\
\hline
1\_UM & 4 & 1\_indomethacin & -0.64 & 0.34 & -1.31 & 0.02 & 0.52 & 0.27 & 1.02 & 0.21 & 0.34 & 0.51\\
\hline
1\_UM & 5 & 1\_indomethacin & -0.06 & 0.45 & -0.93 & 0.81 & 0.94 & 0.39 & 2.25 & 0.28 & 0.49 & 0.69\\
\hline
\end{tabular}
\end{table}
...
The main functions in the package \texttt{pROC} are \texttt{roc}, which creates a \texttt{roc} object, and \texttt{ggroc} which sorts and plots the data for us:
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{ggroc}\NormalTok{(roc\_indo, }\AttributeTok{legacy.axes=}\NormalTok{T) }\SpecialCharTok{+} \FunctionTok{geom\_abline}\NormalTok{(}\AttributeTok{slope=}\DecValTok{1}\NormalTok{, }\AttributeTok{intercept=}\DecValTok{0}\NormalTok{, }\AttributeTok{type=}\DecValTok{2}\NormalTok{) }\SpecialCharTok{+}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/indoroc1-1.pdf}}
...
\NormalTok{indo\_sum }\OtherTok{=}\NormalTok{ indo\_sum }\SpecialCharTok{|\textgreater{}}
  \FunctionTok{filter}\NormalTok{(site }\SpecialCharTok{\%in\%} \FunctionTok{c}\NormalTok{(}\StringTok{"1\_UM"}\NormalTok{, }\StringTok{"2\_IU"}\NormalTok{))}
...
\FunctionTok{ggplot}\NormalTok{(}\AttributeTok{data=}\NormalTok{indo\_sum, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x=}\NormalTok{risk, }\AttributeTok{col=}\NormalTok{rx)) }\SpecialCharTok{+}
  ...
  \FunctionTok{geom\_line}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{x=}\NormalTok{risk, }\AttributeTok{y=}\NormalTok{fit, }\AttributeTok{col=}\NormalTok{rx), }\AttributeTok{linetype=}\DecValTok{2}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{geom\_ribbon}\NormalTok{(}
    \FunctionTok{aes}\NormalTok{(}
      \AttributeTok{x=}\NormalTok{risk, }
      \AttributeTok{ymin=}\NormalTok{fit}\FloatTok{{-}1.96}\SpecialCharTok{*}\NormalTok{fit\_se, }
      \AttributeTok{ymax=}\NormalTok{fit}\FloatTok{+1.96}\SpecialCharTok{*}\NormalTok{fit\_se, }
      \AttributeTok{fill =}\NormalTok{ rx}
\NormalTok{      ),}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/unnamed-chunk-44-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/censor-1.pdf}}
...
\begin{figure}
\includegraphics[width=5.02in]{images/survival_eg} \caption{An example of two survival curves, taken from @syriopoulou2022standardised.}\label{fig:egsurvival}
\end{figure}
...
The \textbf{Kaplan-Meier estimator} is a non-parametric estimate of \(\operatorname{S}\left(t\right)\), originally presented in \citet{kaplan1958nonparametric}. The idea behind it is to divide the interval \(\left[0,\;t\right]\) into many short contiguous intervals,
...
More precisely, let's say that deaths are observed at times \(t_1\leq t_2\leq \ldots \leq t_n\), and that the number of deaths at time \(t_i\) is \(d_i\) out of a possible \(n_i\). Then for some time \(t\in\left[t_J,\,t_{J+1}\right)\), the Kaplan-Meier estimate of \(\operatorname{S}\left(t\right)\) is
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/kmov2-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/ovarian-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/myeloid-1.pdf}}
\caption{\label{fig:myeloid}Kaplan Meier curves for the Myeloid data, split by treatment.}
\end{figure}

\end{example}

\subsection{A parametric approach}\label{surv-exp}
...
Notice that the denominator in Equation \eqref{eq:lambdavar} is \(m\), the number of complete observations (rather than \(n\) the total number including censored observations). This shows that there is a limit to the amount we can learn if a lot of the data is censored.
...
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/ovplotlr-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/myplotlr-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/weibmy-1.pdf}}
...
We can also find a confidence interval for the difference between \(\lambda_T\) and \(\lambda_C\), by using the asymptotic variances of the MLEs, which are \(\frac{\lambda_C^2}{m_C}\) and \(\frac{\lambda_T^2}{m_T}\). Therefore, the limits of a \(100\left(1-\alpha\right)\)\% CI for \(\lambda_T - \lambda_C\) are given by
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Treatment} & \(n_{Tj}-d_{Tj}\) & \(d_{Tj}\) & \(n_{Tj}\) \\
\textbf{Control} & \(n_{Cj}-d_{Cj}\) & \(d_{Cj}\) & \(n_{Cj}\) \\
...
Suppose \(h_C\left(t\right)\) is the hazard at time \(t\) for an individual in group \(C\), and \(h_T\left(t\right)\) is the hazard for that same individual in group \(T\). If the two hazards are proportional then we have (as before)
...
\[\exp\left\lbrace - \int\limits_0^t h_T\left(u\right)du \right\rbrace =\exp\left\lbrace -\int\limits_0^t\psi h_C\left(u\right) du \right\rbrace  \]
From Equation \eqref{eq:sfun} we see that therefore

\[S_T\left(t\right) = \left[S_C\left(t\right)\right]^\psi.\]
Since the survival function is always between 0 and 1, we can see that the value of \(\psi\) determines whether \(S_C\left(t\right)<S_T\left(t\right)\) (if \(0<\psi<1\)) or \(S_C\left(t\right)>S_T\left(t\right)\) (if \(\psi>1\)). The important thing is that \textbf{the survival curves will not cross}. This is an informal conclusion, and lines not crossing is a necessary condition but not a sufficient one.

This property can be helpful in working out what model to fit. For example, it may sometimes be that the survival curves don't cross when a particular {[}influential{]} covariate is factored out, but do when it isn't.
...
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/unnamed-chunk-64-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/loglogov-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/loglogmy-1.pdf}}
\caption{\label{fig:loglogmy}Log-log plot for myeloid data by treatment group.}
\end{figure}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/loglogmy2-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/loglogvet1-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/vetschoen-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/cheesedata-1.pdf}}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{mean}\NormalTok{(cheese}\SpecialCharTok{$}\NormalTok{price)}
\end{Highlighting}
\end{Shaded}
...
Now that we know \(\operatorname{E}\left(\hat{\sigma}^2\right)\) we can adapt our sample size formula from Section \ref{sec-ssformulacont}. For an individual-level RCT with a continuous outcome, and assuming an unpaired, two-sided t-test comparing the outcomes, we had
...
For a cluster RCT, the variance of the treatment effect under the same model assumptions is
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/crtboxplots-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/crt1bp-1.pdf}}
...
To understand mixed effects models, we need to think about the difference between \textbf{fixed effects} and \textbf{random effects}, in particular for categorical/factor variables.
...
\begin{equation}
x_{jk} = \alpha + \beta G_j + \sum\limits_l \gamma_l z_{jkl} + u_{j} + v_{jk}
\label{eq:remodel}
\end{equation}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/eefplot2-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/lmerplot1-1.pdf}}
...
\begin{figure}
\includegraphics[width=5.39in]{images/raneffmodels} \caption{Different ways of including random effects in a mixed effects model.}\label{fig:raneffmodels}
\end{figure}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{mean}\NormalTok{(dfA}\SpecialCharTok{$}\NormalTok{Diff)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{mean}\NormalTok{(dfB}\SpecialCharTok{$}\NormalTok{Diff)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{t.test}\NormalTok{(}\AttributeTok{x=}\NormalTok{dfA}\SpecialCharTok{$}\NormalTok{Diff, }\AttributeTok{y=}\NormalTok{dfB}\SpecialCharTok{$}\NormalTok{Diff, }\AttributeTok{paired=}\NormalTok{F, }\AttributeTok{alternative =} \StringTok{"two.sided"}\NormalTok{, }\AttributeTok{var.equal=}\NormalTok{T)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{drug\_meas }\OtherTok{=} \FunctionTok{c}\NormalTok{(dfA}\SpecialCharTok{$}\NormalTok{Period1,dfB}\SpecialCharTok{$}\NormalTok{Period2)      }\CommentTok{\# Measurements taken while on drug}
\NormalTok{placebo\_meas }\OtherTok{=} \FunctionTok{c}\NormalTok{(dfA}\SpecialCharTok{$}\NormalTok{Period2, dfB}\SpecialCharTok{$}\NormalTok{Period1)  }\CommentTok{\# Measurements taken while on placebo}
\FunctionTok{t.test}\NormalTok{(}
  \AttributeTok{x=}\NormalTok{drug\_meas, }\AttributeTok{y=}\NormalTok{placebo\_meas, }
  \AttributeTok{paired=}\NormalTok{F, }\AttributeTok{alternative=}\StringTok{"two.sided"}\NormalTok{, }\AttributeTok{var.equal=}\NormalTok{T)}
\end{Highlighting}
\end{Shaded}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/epispag-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/epimeans-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/standepi-1.pdf}}
...
\NormalTok{epilepsy}\SpecialCharTok{$}\NormalTok{period }\OtherTok{=} \FunctionTok{as.numeric}\NormalTok{(epilepsy}\SpecialCharTok{$}\NormalTok{period)}
\NormalTok{epi\_1 }\OtherTok{=} \FunctionTok{lmer}\NormalTok{(seizure.rate }\SpecialCharTok{\textasciitilde{}}\NormalTok{ period }\SpecialCharTok{+}\NormalTok{ (}\DecValTok{1}\SpecialCharTok{|}\NormalTok{subject), }\AttributeTok{data=}\NormalTok{epilepsy)}

\DocumentationTok{\#\# Including more baseline covariates}
\NormalTok{epi\_2 }\OtherTok{=} \FunctionTok{lmer}\NormalTok{(seizure.rate }\SpecialCharTok{\textasciitilde{}}\NormalTok{ base }\SpecialCharTok{+}\NormalTok{ age }\SpecialCharTok{+}\NormalTok{ treatment }\SpecialCharTok{+}\NormalTok{ period }\SpecialCharTok{+}\NormalTok{ (}\DecValTok{1}\SpecialCharTok{|}\NormalTok{subject), }\AttributeTok{data=}\NormalTok{epilepsy)}

\NormalTok{pm1 }\OtherTok{=} \FunctionTok{plot\_model}\NormalTok{(epi\_1)}
\NormalTok{pm2 }\OtherTok{=} \FunctionTok{plot\_model}\NormalTok{(epi\_2)}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/unnamed-chunk-74-1.pdf}}
\caption{\label{fig:unnamed-chunk-74}Confidence intervals for fixed effects for the two fitted models.}
\end{figure}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{plot\_model}\NormalTok{(epi\_2)}
\end{Highlighting}
\end{Shaded}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/unnamed-chunk-75-1.pdf}}
\caption{\label{fig:unnamed-chunk-75-1}Confidence intervals for model with same gradient for all participants (left) and for varying gradient (right).}
\end{figure}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{plot\_model}\NormalTok{(epi\_3)}
\end{Highlighting}
\end{Shaded}

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/unnamed-chunk-75-2.pdf}}
\caption{\label{fig:unnamed-chunk-75-2}Confidence intervals for model with same gradient for all participants (left) and for varying gradient (right).}
\end{figure}
...
\addcontentsline{toc}{subsection}{The frequentist approach}
...
\addcontentsline{toc}{subsection}{The Bayesian approach}
...
\addcontentsline{toc}{subsection}{Making probabilistic statements about the treatment effect}
...
\addcontentsline{toc}{subsection}{Sequential analysis}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/betafinal-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/betaplots-1.pdf}}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/betaplotmega-1.pdf}}
...
\addcontentsline{toc}{subsection}{A reference prior}
...
\addcontentsline{toc}{subsection}{A clinical prior}
...
\addcontentsline{toc}{subsection}{A sceptical prior}
...
\addcontentsline{toc}{subsection}{An optimistic / enthusiastic prior}
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{CT4H_notes_files/figure-latex/simplebayes-1.pdf}}
