...
# Computer Practical 1 - Allocation {#cp1allocation}
...
and at [this website](https://www.causeweb.org/tshs/licorice-gargle/), and if you're feeling really keen you can even read the original paper: @ruetzler2013randomized (but please don't do any of that now!).
...
:::{.exercise}
Using the informationn above, bin the two variables `preOp_age` and `preOp_calcBMI` to convert them into factor variables `BMI` and `age`. 
...
:::{.exercise}
Use the `imbalance` function above to find the imbalance in the allocation recorded in the `lic_garg` dataset. Do you find this helpful as a summary of the allocation? 
:::
...
```{r, echo=T}
marg_imbalance(lg_bc1, alloc="treat", factors = lg_factors_all)
```
...
$$2\times{3}\times{4}\times{3}\times{2}\times{3}\times{2} = 864 $$
strata. Even by collecting levels together and so on, we are not going to get down to a sufficiently small number of strata. Therefore we will choose just a couple of covariates to stratify by. For example, gender and smoking status:
...
### Extension: A simulation experiment!
