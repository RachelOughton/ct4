...
## Simple hypothesis tests {#bin-point-est}
...
### A simple method: chi-squared

One way to approach this would be to conduct a **chi-squared** test.

In a chi-squared test, we first calculate the **expected** values $\left(E_i\right)$ in each box of the summary table (ie. the values we'd expect under $H_0$), and compare them to the **observed** values $\left(O_i\right)$ by finding the summary statistic
...
```
 ...
 @altman1998confidence ([available here](https://www.bmj.com/content/bmj/317/7168/1309.full.pdf)) makes a compelling push for the use of confidence intervals for the number needed to treat. You can decide for yourself whether you agree!
...
We will give a sketch of the favourite method of @newcombe1998interval, chosen for its ease of implementation and its accuracy, now. This is the Wilson method, first presented in @wilson1927probable.
...
In Wilson's method, we instead keep $\pi$ in the right hand side and solve the quadratic in Equation \@ref(eq:newcombe1) in terms of $\pi$.
...
```{r newc1, fig.cap="LHS of Equation 6.1 (solid black) with RHS of (black dashed) and RHS with estimate $p$ subbed in (black dotted). The limits of the confidence intervals are where the curves cross, shown by red lines: dashed for Wilson, dotted for standard."}
...
```{r, fig.cap = "As before, dashed for Wilson, dotted for standard"}
qf_pt = quad_fun(
  a = 1 + (qnorm(0.975)^2)/55,
  b = -2*pT - (qnorm(0.975)^2)/55,
  c = pT^2
...
## Accounting for baseline observations: logistic regression {#logreg}
...
## Some things to note:
# There are very few patients from site 4, and not many from site 3
...
print(summary(glm_indo_agelin), digits=2)
vif(glm_indo_agelin)
print(summary(glm_indo_agesq), digits=2)
vif(glm_indo_agesq)

```

Since neither `age` nor `age^2` appear influential, and the AIC and residual deviance values are almost identical, we'll remove it and keep the other covariates.

```{r, echo=T}
glm_indo = glm(outcome ~ site + risk + rx, data=indo_rct, family = binomial(link = "logit"))
print(summary(glm_indo), digits=2)
vif(glm_indo)
```

Notice that

  - The estimate and standard error of $\hat{\tau}$ (the coefficient of `rx`) haven't changed much at all (mainly because, as the VIF shows us, there are no issues with multicollinearity)
...
df_indo$logodds = predict(glm_indo, newdata=df_indo, se.fit = T)$fit
df_indo$logodds_se = predict(glm_indo, newdata=df_indo, se.fit = T)$se.fit
df_indo$logor_lower = df_indo$logodds - qnorm(0.975)*df_indo$logodds_se
df_indo$logor_upper = df_indo$logodds + qnorm(0.975)*df_indo$logodds_se

df_indo$odds = exp(df_indo$logodds)
df_indo$odds_lower = exp(df_indo$logor_lower)
df_indo$odds_upper = exp(df_indo$logor_upper)

df_indo$prob_lower = df_indo$odds_lower/(1+df_indo$odds_lower)
df_indo$prob = df_indo$odds/(1+df_indo$odds)
df_indo$prob_upper = df_indo$odds_upper/(1+df_indo$odds_upper)
...
## Diagnostics for logistic regression {#diaglogreg}
...
ROC_allT_plot = function(
  ...
  alpha0=pars[1]   # mean and SD for X=0
  beta0=pars[2]
  alpha1=pars[3]  # mean and SD for X=1
  beta1=pars[4]
...
ROC_dist_plot = function(
  ...
  dist_plot = ggplot() + xlim(0,1) +
    ...
    ggtitle(sprintf("Sensitivity = %g, Specificity = %g",
...
param_df = data.frame(
  alpha0 = c(1.3, 2.2, 3),
  beta0 = c(5.5, 4.1, 3.6),
  alpha1 = c(5.5, 3.9, 2.2),
  beta1 = c(1.3, 2.3, 1.3)
)

```

```{r rocgood, fig.cap = "ROC example with good separation."}

gg_dist_good = ROC_dist_plot(as.numeric(param_df[1, ]), thresh=0.5)
gg_roc_good = ROC_allT_plot(as.numeric(param_df[1, ]))
grid.arrange(gg_dist_good, gg_roc_good, nrow=1)


```

```{r rocpoor, fig.cap = "ROC example with poor separation."}
gg_dist_poor = ROC_dist_plot(as.numeric(param_df[3, ]), thresh=0.5)
gg_roc_poor = ROC_allT_plot(as.numeric(param_df[3, ]))
grid.arrange(gg_dist_poor, gg_roc_poor, nrow=1)
```
...
The main functions in the package `pROC` are `roc`, which creates a `roc` object, and `ggroc` which sorts and plots the data for us:
...
```{r indoroc1, echo=T, fig.cap = "ROC curve for our logistic regression model of the indo RCT data (solid line). The line y=x shows the ROC curve we'd expect with random guessing."}
...
ggplot(data=indo_sum, aes(x=risk, col=rx)) +
  ...
  geom_line(aes(x=risk, y=fit, col=rx), linetype=2) +
  geom_ribbon(
    aes(
      x=risk, 
