...
if(knitr::is_html_output()){
  ...
  knitr::include_graphics("images/survival_eg.png", dpi=200) 
}
```
...
The **Kaplan-Meier estimator** is a non-parametric estimate of $\operatorname{S}\left(t\right)$, originally presented in @kaplan1958nonparametric. The idea behind it is to divide the interval $\left[0,\;t\right]$ into many short contiguous intervals,
...
### A parametric approach {#surv-exp}
...
-----------------  -----------------  -----------   --------------
    **Treatment**  $n_{Tj}-d_{Tj}$    $d_{Tj}$       $n_{Tj}$     
      **Control**  $n_{Cj}-d_{Cj}$    $d_{Cj}$       $n_{Cj}$    
...
logrank_df |> kable() |> kable_styling("striped")
```
